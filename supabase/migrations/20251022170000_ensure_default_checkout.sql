-- Migration: Garantir que todo produto tenha checkout padrão
-- Data: 2025-10-22
-- Descrição: Cria automaticamente um checkout padrão quando uma oferta padrão é criada

-- Função para criar checkout padrão quando oferta padrão é criada
CREATE OR REPLACE FUNCTION create_default_checkout_for_offer()
RETURNS TRIGGER AS $$
DECLARE
  v_product_name TEXT;
  v_checkout_id UUID;
  v_link_id UUID;
  v_checkout_count INTEGER;
BEGIN
  -- Só executar se for oferta padrão
  IF NEW.is_default = true THEN
    -- Verificar se o produto já tem algum checkout
    SELECT COUNT(*) INTO v_checkout_count
    FROM checkouts
    WHERE product_id = NEW.product_id;
    
    -- Se não tem nenhum checkout, criar um padrão
    IF v_checkout_count = 0 THEN
      -- Buscar nome do produto
      SELECT name INTO v_product_name
      FROM products
      WHERE id = NEW.product_id;
      
      -- Criar checkout padrão
      INSERT INTO checkouts (product_id, name, is_default)
      VALUES (NEW.product_id, v_product_name || ' - Checkout Padrão', true)
      RETURNING id INTO v_checkout_id;
      
      -- Buscar o link da oferta padrão
      SELECT id INTO v_link_id
      FROM payment_links
      WHERE offer_id = NEW.id;
      
      -- Associar o link ao checkout
      IF v_link_id IS NOT NULL THEN
        INSERT INTO checkout_links (checkout_id, link_id)
        VALUES (v_checkout_id, v_link_id);
      END IF;
    END IF;
  END IF;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Criar trigger para executar após inserção de oferta
DROP TRIGGER IF EXISTS create_default_checkout_trigger ON offers;
CREATE TRIGGER create_default_checkout_trigger
  AFTER INSERT ON offers
  FOR EACH ROW
  EXECUTE FUNCTION create_default_checkout_for_offer();

-- Atualizar função de criação de link para também criar checkout se necessário
CREATE OR REPLACE FUNCTION auto_create_payment_link()
RETURNS TRIGGER AS $$
DECLARE
  v_slug TEXT;
  v_base_url TEXT;
  v_link_id UUID;
BEGIN
  -- Gerar slug único
  v_slug := generate_link_slug(NEW.name, NEW.price);
  
  -- URL base da aplicação
  v_base_url := 'https://risecheckout.lovable.app/c/';
  
  -- Inserir o link
  INSERT INTO payment_links (offer_id, slug, url)
  VALUES (NEW.id, v_slug, v_base_url || v_slug)
  RETURNING id INTO v_link_id;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Comentários
COMMENT ON FUNCTION create_default_checkout_for_offer() IS 
'Cria automaticamente um checkout padrão quando uma oferta padrão é criada para um produto sem checkouts';

COMMENT ON TRIGGER create_default_checkout_trigger ON offers IS 
'Garante que todo produto tenha pelo menos um checkout padrão associado à oferta padrão';

